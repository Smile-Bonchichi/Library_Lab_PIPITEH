...
Tree* pop(Tree* Btree, int x) {
	...
	if ((x == Btree->left->item) && (Btree->left->left == nullptr) && (Btree->left->right == nullptr)) {
		delete Btree->left;
		delete Btree->right;
		Btree->left = Btree->right = NULL;
		return Btree;
	}
	else if ((x == Btree->right->item) && (Btree->right->left == nullptr) && (Btree->right->right == nullptr)) {
		delete Btree->right;
		delete Btree->left;
		Btree->left = Btree->right = NULL;
		return Btree;
